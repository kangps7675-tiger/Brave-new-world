...
export async function rewriteLampNarrative(
  ...
  const system = korean
    ? [
        ...
        "불가피한 고유명사와 초크포인트·OSINT·AIS처럼 한국어로도 잘 통하는 외래어는 그대로 쓰되, 주어·조사·서술어가 갖춰진 문장에 넣으십시오. 생소한 영문 약어는 첫 등장에 한글 뜻을 덧붙이십시오.",
        "JSON 이외의 설명이나 마크다운을 출력하지 마십시오.",
      ].join("\n")
    : [
