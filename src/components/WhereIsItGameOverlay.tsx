...
export function WhereIsItGameOverlay({ lang, pool, onFlyTo, onClose }: Props) {
  ...
  return (
    <div
      className="pointer-events-none fixed inset-x-0 bottom-0 z-[800] flex justify-center p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] sm:p-4"
      role="dialog"
      aria-modal="false"
    ...
    >
      <div className="pointer-events-auto w-full max-w-md rounded-2xl border border-amber-400/30 bg-[#0a1220]/94 shadow-2xl backdrop-blur-xl">
        <div className="flex items-start justify-between gap-2 border-b border-white/10 px-4 py-3">
          <div>
            <p className="text-meta font-semibold uppercase tracking-[0.16em] text-amber-300/90">
        ...
        {phase === "done" ? (
          <div className="space-y-3 px-4 py-4">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 rounded-xl border border-amber-400/40 bg-amber-500/15 px-3 py-2.5 text-body font-semibold text-amber-100"
              >
                {ko ? "한 판 더" : "Play again"}
              </button>
              <button
          ...
          <div className="px-4 py-3">
            {spot ? (
              <p className="mb-2 text-meta text-slate-500">
            ...
            <div className="grid grid-cols-2 gap-2">
              {choices.map((c) => {
                ...
                let cls =
                  "rounded-xl border px-2.5 py-2.5 text-left text-caption font-medium transition ";
                if (phase === "reveal") {
            ...
            {phase === "reveal" ? (
              <button
                type="button"
                onClick={handleNext}
                className="mt-3 w-full rounded-xl border border-amber-400/40 bg-amber-500/15 px-3 py-2.5 text-body font-semibold text-amber-100"
              >
