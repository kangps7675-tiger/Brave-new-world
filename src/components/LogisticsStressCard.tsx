  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-3">
        <p className="text-body font-semibold text-slate-100">{title}</p>
        <span
          className="rounded-full px-2.5 py-0.5 text-meta font-semibold"
        ...
        <ul className="mt-3 space-y-2">
          {stress.signals.map((s, i) => (
            <li key={i} className="flex items-start gap-2 text-caption leading-snug">
              <span
                className="mt-0.5 shrink-0 rounded px-1 py-[1px] text-micro font-bold"
                style={{
              ...
              <span className="text-slate-200/90">
                ...
                {s.isDemo ? (
                  <span className="ml-1 rounded bg-fuchsia-500/20 px-1 text-micro font-bold text-fuchsia-300">
                    DEMO
                  </span>
                ) : null}
      ...
      {!stress.graded ? (
        <p className="mt-2.5 text-meta text-slate-500">
          {en
